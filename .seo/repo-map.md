...
## Alerte haute n°2 — écart factuel sur la loi citée dans le prompt de référence

Le prompt de référence (et donc les gabarits d'articles réglementaires) cite systématiquement la **« Loi n°35-09 »** comme texte régissant les activités privées de sécurité au Maroc. La recherche web de ce run (sources : Bulletin Officiel, SGG, plusieurs cabinets et blogs juridiques marocains) indique de façon convergente que la loi réellement en vigueur sur le gardiennage et le transport de fonds au Maroc est la **loi n° 27-06** (promulguée par le dahir n° 1-07-155, 2007), et non une « loi 35-09 ». Aucune source consultée ne mentionne de « loi 35-09 » relative à la sécurité privée.

Par prudence (§8.3, règle non négociable : aucune citation réglementaire sans confirmation par une source primaire de la liste autorisée), **cet agent n'a produit aucun contenu citant un numéro de loi ce run**, et a évité le cluster 8 (Loi n°35-09) et le hub conformité. Cette incohérence doit être vérifiée par un humain avant toute production de contenu réglementaire : soit le prompt de référence contient une erreur à corriger (numéro probable : 27-06), soit il existe une loi 35-09 distincte non retrouvée par la recherche de ce run et qu'il faudrait sourcer précisément. Tant que ce point n'est pas tranché, le hub conformité et le cluster 8 restent bloqués.
